...
import Mdbv1Edit from "./Mdbv1Edit";
import Mdbv1ModalDel from "./Mdbv1ModalDel";
import axios from "axios";
import { toast, Toaster } from "sonner";
import { Link } from "react-router";

export default function Mdbv1() {
  const { data, getData, loadData, errData } = useMdbv1();
                  ...
                  <div className="flex justify-between items-start gap-2">
                    ...
                    {editId === item._id ? (
                      ...
                      <div className="flex gap-4">
                        ...
                        <Mdbv1ModalDel item={item} delId={delId} setDelId={setDelId} />
                      </div>
                    )}
                  </div>
                </div>
                {/* EDIT */}
                <div>{editId === item._id && <Mdbv1Edit item={item} setEditId={setEditId} />}</div>
              </React.Fragment>
            ))}
        </div>
      </div>
