...
import { Seqv2Products } from "./typesSeqv2";
import { url, useSeqv2 } from "./useSeqv2";
import { toast } from "sonner";
import axios from "axios";
import { FaXmark } from "react-icons/fa6";

interface Seqv2ModalDelProps {
  item: Seqv2Products;
  delId: string | null;
  setDelId: React.Dispatch<React.SetStateAction<string | null>>;
}
export default function Seqv2ModalDel({ item, delId, setDelId }: Seqv2ModalDelProps) {
  const [loadDel, setLoadDel] = useState(false);

  const { getData } = useSeqv2();
  const onDelete = async () => {
    setLoadDel(true);
    axios
      .delete(`${url}/api-sequelize/v2/product/${item.id}`)
      .then((res) => {
