...
import Moov3CategoriesModalDel from "./Moov3CategoriesModalDel";
import Moov3CategoriesEdit from "./Moov3CategoriesEdit";

export default function Moov3Categories() {
  const { categories, getCategories, loadCategories } = useMoov3();
  ...
  const onCreate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios
      ...
      .post(`${url}/api-mongoose/v3/category`, { name })
      .then((res) => {
        ...
        <div className="flex items-center justify-between">
          ...
          {editId === category._id ? (
            ...
            <div className="flex gap-3">
              <button type="button" title="update" onClick={() => setEditId(category._id)}>
                <FaPenToSquare className="text-green-500" />
              </button>
              <button type="button" title="delete" onClick={() => setDelId(category._id)}>
                <FaTrash className="text-red-500" />
              </button>
              <Moov3CategoriesModalDel item={category} delId={delId} setDelId={setDelId} />
            </div>
          )}
        </div>
        {editId === category._id && <Moov3CategoriesEdit item={category} setEditId={setEditId} />}
      </div>
    ));
  }
